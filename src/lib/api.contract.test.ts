...
const backendAvailable = goAvailable && existsSync(path.join(backendRoot, 'go.mod'));
...
async function buildAndStart(): Promise<string> {
  ...
  execFileSync(path.join(binDir, 'e2e-seed'), [
    '-db',
    path.join(homeDir, '.nexus', 'applications.db'),
  ], {
    cwd: backendRoot,
  ...
  child = spawn(path.join(binDir, 'nexus'), ['--api', '--api-port', String(port)], {
    cwd: backendRoot,
    env: { ...process.env, HOME: homeDir },
...
describe.skipIf(!backendAvailable)('Nexus API contract (real Go backend)', () => {
  beforeAll(async () => {
